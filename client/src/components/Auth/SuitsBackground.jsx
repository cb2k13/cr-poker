...
function mkSpade() {
  const s = new THREE.Shape();
  s.moveTo(0, 0.52);
  // Right arch (inverted heart right side)
  s.bezierCurveTo(0.6, 0.52, 0.68, 0, 0.42, -0.22);
  // Corner into right stem edge
  s.lineTo(0.25, -0.28);
  // Stem right, down
  s.lineTo(0.25, -0.52);
  // Stem bottom
  ...
  // Stem left, up
  s.lineTo(-0.25, -0.28);
  // Corner out to left lobe
  s.lineTo(-0.42, -0.22);
  // Left arch (mirror)
  s.bezierCurveTo(-0.68, 0, -0.6, 0.52, 0, 0.52);
  return new THREE.ExtrudeGeometry(s, EXT);
}
...
const SUITS = [
  ...
  // spades — black
  { type: 'spade',   position: [-6,   2,  -3],    rotation: [0.2, 0.8,-0.1],   scale: 1.3, color: '#111111', speed: 0.26, offset: 0.4  },
  { type: 'spade',   position: [7,   -3,  -4],    rotation: [-0.3,0.2, 0.4],   scale: 1.5, color: '#111111', speed: 0.22, offset: 1.5  },
  { type: 'spade',   position: [1,    6.5,-5],    rotation: [0.6, 0.1, 0.3],   scale: 1.1, color: '#111111', speed: 0.30, offset: 3.2  },
  { type: 'spade',   position: [-2,  -4,  -2],    rotation: [0.1,-0.5, 0.2],   scale: 1.4, color: '#111111', speed: 0.36, offset: 4.8  },
  { type: 'spade',   position: [9,    3,  -6],    rotation: [-0.2,0.4, 0.6],   scale: 1.6, color: '#111111', speed: 0.19, offset: 5.7  },
  // clubs — black
  { type: 'club',    position: [-4,   4.5,-4],    rotation: [0.4, 0.3,-0.2],   scale: 1.2, color: '#111111', speed: 0.24, offset: 0.9  },
  { type: 'club',    position: [3.5, -4,  -3],    rotation: [0.1, 0.6, 0.3],   scale: 1.4, color: '#111111', speed: 0.29, offset: 2.0  },
  { type: 'club',    position: [-9,  -2,  -5],    rotation: [-0.3,0.1, 0.5],   scale: 1.7, color: '#111111', speed: 0.17, offset: 3.5  },
  { type: 'club',    position: [5.5,  5,  -2],    rotation: [0.5,-0.3, 0.1],   scale: 0.9, color: '#111111', speed: 0.40, offset: 4.1  },
  { type: 'club',    position: [-1,  -6,  -6],    rotation: [0.2, 0.4,-0.4],   scale: 1.5, color: '#111111', speed: 0.21, offset: 6.0  },
];
...
function FloatingSuit({ type, position, rotation, scale, color, speed, offset }) {
  ...
  const matProps = {
    color,
    metalness: isBlack ? 0.7 : 0.35,
    roughness:  isBlack ? 0.2 : 0.45,
    transparent: true,
    opacity: isBlack ? 0.85 : 0.72,
  };
...
function Scene() {
  return (
    <>
      ...
      <pointLight position={[5, 8, 6]}   intensity={3.5} color="#ffffff" />
      <pointLight position={[-8, -4, 2]} intensity={2.0} color="#ffffff" />
      <pointLight position={[0, 0, 8]}   intensity={1.0} color="#d4af37" />
      {SUITS.map((props, i) => (
